from modelos_polimorfismo.avaliacao import Avaliacao
...
class Restaurante:
    ...
    def exibir_cardapio(self):
        #return f'Cardápio Restaurante {self._nome}' + (item for i, item in enumerate (self._cardapio, start = 1)) 
        print(f'Cardápio Restaurante {self._nome}\n' + '\n'.join(f'{i}: {item}' for i, item in enumerate(self._cardapio, start=1)))
       
        # if hasattr(item,"descrição"): - verifica se o objeto possui o atributo passado
        #     mensagemPrato
        # else:
        #     mensagemBebida